...
error() {
    echo -e "[ERROR]\t$1\n" | tee -a $LOGFILE >&2
    echo -e "\t\t usage: $0 \'image-path\' \'device-mount-point\' \n" | tee -a $LOGFILE 2>&1
    exit 1
}
...
## Check/prep directories
    ...
    DEVFILE=$(lsblk -o KNAME,PATH,MODEL,MOUNTPOINT,UUID | grep $DEV_I | awk {'print $2'}) 2>&1
    if [ -z $DEVFILE ]; then
    		error "$DEV_I: Block device not found"
    else
    		echo "$DEV_I Located at $DEVFILE"
    fi
